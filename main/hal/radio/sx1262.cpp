...
namespace HAL
{
    ...
    SX1262::SX1262(const SX1262Pins& pins)
        : _pins(pins), _spi_handle(nullptr), _spi_mutex(nullptr), _mode(RadioMode::SLEEP), _state(SX1262State::UNINITIALIZED),
          ...
          _active_receive_start_ms(0), _cad_rx_start_ms(0), _initialized(false)
    {
    ...
    bool SX1262::init()
    {
        ...
        // Configure IRQ: route actionable completion events to DIO1
    ...
    bool SX1262::startCAD()
    {
        ...
        // Pre-configure RX packet parameters and boosted gain so that if CAD detects activity
        // and transitions to RX (cadExitMode = 0x01), packet demodulation succeeds with correct params.
        setPacketParams(_config.preamble_length, _config.implicit_header, 255, _config.crc_enabled, _config.iq_inverted);
        waitBusy();
